...
# setting the data from the config file
...
if __name__ == "__main__":
    ...
    class CustomCheckpointCallback(Callback):
        def __init__(self, save_interval, ckptdir):
        ...
        def on_epoch_end(self, trainer, pl_module):
            ...
            if epoch % self.save_interval == 0:
                ckpt_path = os.path.join(self.ckptdir, f"epoch_{epoch:06d}.ckpt")
                trainer.save_checkpoint(ckpt_path)
                print(f"Checkpoint saved at {ckpt_path}")

    # Update your trainer_kwargs to include the custom callback
    custom_ckpt_callback = CustomCheckpointCallback(save_interval=1000, ckptdir=ckptdir)

    trainer_kwargs["callbacks"].append(custom_ckpt_callback)
    ...
    # Maxepochs:
    # print('epoch:', opt.epoch, type(opt.epoch))
