...
class AlignmentBackboneDiT(nn.Module):
    """
    ...
    def __init__(
        ...
        self.pos_emb_vid = PositionEmbedding(num_embeddings=max_len,embedding_dim = hidden_size)
        self.pos_emb_aud = PositionEmbedding(num_embeddings=max_len,embedding_dim = hidden_size)
        ...
        self.pool_out = nn.AdaptiveAvgPool1d(1)
    ...
    def forward(self, x, t, context):
        ...
        c = self.pos_emb_vid(c)
        extra_len = c.shape[1] + 1

        x = self.proj_in(x)
        x = rearrange(x, 'b c t -> b t c')
        x = self.pos_emb_aud(x)

        xc = torch.concat([t,c,x], dim=1)
        ...
        xc = self.pool_out(xc).transpose(2, 1)
        xc = self.final_layer(xc)                # (N, out_channels,T)
        pred = self.sigmoid(xc)
        
        return pred


class AlignmentBackboneDiTConcat(nn.Module):
    """
    Diffusion model with a Transformer backbone.
    """
    def __init__(
...
class AlignmentBackboneDiTConcatStartPoint(nn.Module):
    """
    Diffusion model with a Transformer backbone.
    """
    def __init__(
        ...
        self.proj_in_x = nn.Conv1d(in_channels,hidden_size,kernel_size=kernel_size,padding=kernel_size//2)
        self.proj_in_v = nn.Conv1d(in_channels,hidden_size,kernel_size=kernel_size,padding=kernel_size//2)

        self.x_proj = nn.Linear(hidden_size, hidden_size // 3)
        self.v_proj = nn.Linear(hidden_size, hidden_size // 3)
        self.c_proj = nn.Linear(hidden_size, hidden_size // 3)
    ...
    def forward(self, v, t, x, context):
        """
        Forward pass of DiT.
        x: (N, C, T) tensor of temporal inputs (latent representations of melspec)
        t: (N,) tensor of diffusion timesteps
