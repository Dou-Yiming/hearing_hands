...
from adm.models.diffusion.sd_ddim_maa2_scale import DDIMSampler
from adm.models.diffusion.sd_ddpm_maa2_scale import DDPM
...
class Wrapper_CFG(nn.Module):
    def __init__(self, net, cond, uncond_cond, cfg_scale):
        ...
        self.net = net
        self.cond = cond
        if isinstance(uncond_cond, dict):
            for k, v in uncond_cond.items():
                uncond_cond[k] = v.to(cond.device)
        self.uncond_cond = self.net.get_learned_conditioning(
            uncond_cond)
...
class CFM_MAA2_CFG(DDPM):
    ...
    def __init__(self,
        ...
        if 'model_ckpt_path' in kwargs:
            ...
            ignore_keys = ['diffusion_model.c1_embedder',
                           'diffusion_model.c2_embedder']

            self.init_diffusion_model_ckpt(
                kwargs['model_ckpt_path'], ignore_keys)
        ...
        self.zero_embed_dict = {
            'mix_video_feat': self.zero_embed,
            'mix_hand_pose': torch.zeros(1, 240, 126),
            'mix_video_frames': torch.zeros(1, 32, 3, 224, 224),
        }
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        if isinstance(encoder_posterior, DiagonalGaussianDistribution) or str(type(encoder_posterior)) == "<class 'ldm.modules.distributions.distributions.DiagonalGaussianDistribution'>":
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox']:
                    ...
                    xc = batch
                elif type(cond_key) is not str:
                    xc = {k: batch[k] for k in cond_key}
                else:
        ...
        if train:
            if not isinstance(c, dict):
                new_bs = z.shape[0]
                video_feat_mask = self.prob_mask_like(
                    (new_bs, ), 1 - self.class_free_drop_prob, device=self.device)
                ...
                for k, v in c.items():
                    # st()
                    null_embed = torch.zeros(1, *v.shape[1:]).to(self.device)
                    if k not in self.zero_embed_dict:
                        self.zero_embed_dict[k] = null_embed
                    video_feat_mask = video_feat_mask.reshape(new_bs, *[1] * (v.dim() - 1))
                    c[k] = torch.where(video_feat_mask, c[k], null_embed)
    ...
    def ode_wrapper_cfg(self, cond, cfg_scale):
        if isinstance(self.cond_stage_key, str):
            ...
            return Wrapper_CFG(self, cond, self.zero_embed_dict, cfg_scale)
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
        ...
        neural_ode = NeuralODE(self.ode_wrapper(
            cond), solver='euler', sensitivity="adjoint", atol=1e-4, rtol=1e-4)
    ...
    def sample_param(self, cond, batch_size=16, return_intermediates=False, x_T=None,
        ...
        neural_ode = NeuralODE(self.ode_wrapper(
            cond), solver=solver, sensitivity="adjoint", atol=1e-4, rtol=1e-4)
    ...
    def sample_param_cfg(self, cond, cfg_scale=4.5, batch_size=16, return_intermediates=False, x_T=None,
        ...
        if cond is not None:
            if isinstance(cond, dict):
                ...
                cond = [c[:batch_size] for c in cond] if isinstance(
                    cond, list) else cond[:batch_size]
        neural_ode = NeuralODE(self.ode_wrapper_cfg(
            cond, cfg_scale), solver=solver, sensitivity="adjoint", atol=1e-4, rtol=1e-4)
        t_span = torch.linspace(0, 1, timesteps)
        x0 = torch.randn(shape, device=self.device)
        eval_points, traj = neural_ode(x0, t_span)
