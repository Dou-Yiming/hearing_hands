...
class ConcatDiT(nn.Module):
    ...
    def forward(self, x, t, context):
        ...
        x = self.proj_in(x)
        x = rearrange(x,'b c t -> b t c') 
        x = torch.concat([t,c,x],dim=1) # 在时间维度上concat到一起了
        x = self.pos_emb(x)
        x = rearrange(x,'b t c -> b c t')
...
class ChannelConcatDiTNoUpsample(nn.Module):
    """
    Diffusion model with a Transformer backbone.
    """
    def __init__(
...
class ChannelConcatDiTMavil(nn.Module):
    """
    Diffusion model with a Transformer backbone.
    """
    def __init__(
        ...
        self.c_upsampler = nn.Upsample(scale_factor=16, mode='nearest')
        self.c_embedder = ConditionEmbedder(hidden_size, context_dim)
